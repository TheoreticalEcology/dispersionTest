...
#trying to generate underdispersed data

dataOK    <- createData2(100, randomEffectVariance = 0, family=compois())

dataUnder <- createData2(100, randomEffectVariance = 0, family=compois(), 
                         nu.cmp=2.5)


ggplot(dataUnder, aes(x=Environment1, y=observedResponse)) +
  geom_point(col="red") +
  geom_smooth(method="glm", method.args=list(family=poisson()),col="red")+
  geom_point(data=dataOK, aes(x=Environment1, y=observedResponse)) +
  geom_smooth(data=dataOK, method="glm", method.args=list(family=poisson()),
              col="black")

mod <- glm(observedResponse ~ Environment1, data=dataUnder)
res <- simulateResiduals(mod)
testDispersion(res)
testDispersion(mod, type = "PearsonChisq")
...
createData2 <- function(sampleSize = 100, intercept = 0, fixedEffects = 1,
  ...
  for (i in 1:replicates){
    ...
    else if(family$family == "compois") {
      observedResponse = COMPoissonReg::rcmp(n=sampleSize, lambda = linkResponse, nu=nu.cmp)
