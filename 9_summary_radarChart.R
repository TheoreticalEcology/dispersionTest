...
df <- tibble(
  ...
  Speed = c(5,2,3),
  GLM = c(5,5,5),
  `GLM (small-data)` = c(1,4,3),
  `GLMM (few RE groups)` = c(2,5,4),
  `GLMM (many RE groups)` = c(1,4,4)
)
...
ggplot(df2, aes(x=variable.n, y=test.n, size=value, fill=value, col=value))+
  geom_point(shape=21,) +
...
ggsave(filename = "figures/9_testsEvaluation.jpeg", height = 5, width = 9)
