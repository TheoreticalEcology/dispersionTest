...
load(here("data", "5_glmmBin_power_OLD.Rdata")) # simulated data
#simulations
...
ggsave(here('figures', '7_aprroxPearson_bin_powerGLMM.jpeg'), height=10, width=10)
```
...
load(here("data", "5_glmmPois_power_OLD.Rdata")) # simulated data
...
poisdata %>% filter(ngroups==100, intercept %in% c("-1.5", "0", "1.5"),
  ...
  theme(panel.background = element_rect(color="black"),
        legend.position = "bottom") + 
  guides(color=guide_legend(nrow=2, byrow=TRUE))
ggsave(here('figures', '7_aprroxPearson_pois_powerGLMM.jpeg'), height=10, width=10)
```
